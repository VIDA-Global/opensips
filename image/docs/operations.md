...
The AMI installs `/etc/opensips/opensips.cfg.template`; operators do not supply arbitrary OpenSIPS text. Create one schema-v2 deployment object per node from `config/deployment.json.example`. Give each node a unique `node_id`, `private_ip` and placement service token. Keep the placement namespace shared across the cohort. Set `state_owner` to `active` on prepared nodes: the independent [ownership controller](ownership.md) grants SQL authority before the native startup guard permits execution, and standbys remain cold. These grants are separate from placement observer leases. Keep `cluster_id`, advertised identity, carrier allowlists, RTPengine nodes and TLS policy equivalent. The frontend must preserve SIP flow affinity; established-dialog restoration still requires deployment qualification.
...
The renderer accepts only schema version 2. Its deployment object includes the exact fields in `config/deployment.json.example`, including `placement` and `voice_ingress_namespace`. The TLS object must contain exactly `certificate`, `private_key`, and `ca_bundle`. See the README schema table for types and bounds. Legacy `opensips_config` payloads and unknown extension fields are rejected.
